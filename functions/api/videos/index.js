...
export async function onRequestPost({ env, request }) {
  ...
  const { meta } = await env.DB.prepare(
    'INSERT INTO videos (title, description, bunny_video_id, thumbnail_url, duration_seconds, tag, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).bind(title, description || '', bunny_video_id, thumbnail_url || '', safeDuration, (tag || '').slice(0, 60), Date.now()).run();
  return json({ id: meta.last_row_id, title, bunny_video_id, duration_seconds: safeDuration, tag: (tag || '').slice(0, 60) }, 201);
}
