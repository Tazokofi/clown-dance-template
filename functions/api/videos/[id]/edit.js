...
export async function onRequestPost({ params, env, request }) {
  ...
  await env.DB.prepare(
    'UPDATE videos SET title = ?, description = ?, bunny_video_id = ?, thumbnail_url = ?, duration_seconds = ?, tag = ?, orientation = ? WHERE id = ?'
  ).bind(title, description || '', bunny_video_id, thumbnail_url || '', safeDuration, safeTag, safeOrientation, video_id).run();

  return json({
    id: video_id, title, description: description || '',
    bunny_video_id, thumbnail_url: thumbnail_url || '', duration_seconds: safeDuration, tag: safeTag, orientation: safeOrientation
  });
}
