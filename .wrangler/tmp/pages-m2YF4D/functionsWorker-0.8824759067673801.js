...
// api/admin/ban.js
async function onRequestPost7({ env: env2, request }) {
  const user = await getUserFromRequest(request, env2.DB);
...
// api/auth/signup.js
async function onRequestPost11({ env: env2, request }) {
...
async function onRequestGet3({ env: env2 }) {
  const { results } = await env2.DB.prepare(
    "SELECT id, title, description, bunny_video_id, thumbnail_url, view_count, like_count, dislike_count, comment_count, created_at FROM videos ORDER BY created_at DESC"
  ).all();
  return json(results);
}
...
// ../.wrangler/tmp/pages-m2YF4D/functionsRoutes-0.8676152534840966.mjs
var routes = [
  {
